...
def cli(ctx):
    ...
    *** CLTV CLI ***
    ...
    Making routine cltv tasks a little easier.
...
@click.option(
    ...
    default=False,
    help=(
        "Removes previously-installed cltv libs (for clean-up only; "
        "not required for your newly-installed lib to work; databricks "
        "will use the latest cltv lib by default)."
    ),
)
@click.pass_context
def install_on_db(ctx, cluster_id, remove_old):
    """Installs your local cltv code onto databricks.
    ...
    install_lib_on_databricks(cluster_id, "cltv", remove_old)
...
def upload_to_blob(ctx):
    """
    Uploads the wheel file to Azure Dev Blob for testing.
    ...
    for file in files:
        file_name = f"adhoc/{user}/libraries/cltv/{file.split('/')[-1]}"
        upload_release_file_to_blob(file, file_name)
...
def upload_to_prod(ctx):
    """
    Uploads the wheel file to production library folder to be used by our jobs.
    ...
    for file in files:
        file_name = f"libraries/cltv/libs/{file.split('/')[-1]}"
        upload_release_file_to_blob(file, file_name, blob)
